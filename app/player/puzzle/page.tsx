  ...
  const handleComplete = useCallback(
    async (score: number, timeSpent: number, moves: number) => {
      ...
      const updatedPlayedTypes = Array.from(
        new Set([...player.playedGameTypes, 'puzzle'])
      )
      ...
      const earnedIds = checkAchievements(
        { gameType: 'puzzle', score, timeSpent, moves, difficulty },
        ownedIds,
        updatedGamesPlayed,
  ...
  return (
    <div className="min-h-screen bg-[#000009] p-6">
      <div className="max-w-4xl mx-auto">
        <Link href="/player">
          <button className="flex items-center gap-2 text-slate-400 hover:text-pink-400 transition-colors mb-8">
            <ArrowLeft className="w-5 h-5" />
            Back to Games
          </button>
        </Link>

        <h1 className="text-3xl font-bold text-pink-300 mb-8 text-center">Puzzle Game</h1>
        ...
        {minting ? (
          ...
          <PuzzleGame onComplete={handleComplete} />
        )}
      </div>
